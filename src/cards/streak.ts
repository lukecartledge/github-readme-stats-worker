import type { StreakData, StreakCardOptions } from './types.js'
...
const safeDateFormat = (date: string | undefined, locale: string | null | undefined): string => {
  if (!date) {
    return 'N/A'
  }
...
const getStyles = ({ titleColor, textColor }: { titleColor: string; textColor: string }): string => {
  return `
    .title {
...
const renderColumn = ({ x, title, value, subTop, subBottom }: { x: number; title: string; value: number | string; subTop: string; subBottom: string }): string => {
  return `
...
const renderStreakCard = (data: StreakData, options: Partial<StreakCardOptions> = {}): string => {
