 ...
 */
const getTotalCommitsYearLabel = (include_all_commits: boolean, commits_year: number | undefined, i18n: InstanceType<typeof I18n>): string =>
  include_all_commits
    ? ''
    : commits_year
      ? ` (${commits_year})`
...
import type { StatsData, StatCardOptions } from './types.js'
...
const renderStatsCard = (stats: StatsData, options: Partial<StatCardOptions> = {}): string => {
  ...
  const STATS: Record<string, { icon: string; label: string; value: number | string; id: string; unitSymbol?: string }> = {}
  ...
  const statItems = Object.keys(STATS)
    ...
    .map((key, index) => {
        const stats = STATS[key] as { icon: string; label: string; value: number | string; id: string; unitSymbol?: string }
